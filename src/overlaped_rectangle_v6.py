...
def plot_the_figure(coordinates, points_coords):
    ...
    for points_array in points_coords:
        for p in points_array:
            if p[1]:
                ax.plot(p[0][0], p[0][1], 'o', color='green')
            else: 
                ax.plot(p[0][0], p[0][1], 'o', color='red')  
    ...
    # Display the plot
    #plt.show()
def get_the_eopatch(bbox):
    # Set up Sentinel Hub credentials
    config = SHConfig()
    #config.instance_id = 'your_instance_id'
    config.sh_client_id = '42a58853-815f-4150-b403-83c8509868c2'
    config.sh_client_secret = 'Ztr74Z02KbcuYLlTxohYk80LbX3P8FsS'
...
for el in points_array:
    points_array_utm.append([transformer.transform(lat, lon) for lat, lon in el])
...
eopatch = get_the_eopatch (bbox)

plot_the_result(eopatch)
